...
{
    // If you want to add another translation, simply add a language here with its code
...
void Translator::translateApplication(QApplication* app, QString language)
{
    if (isTranslationAvailable(language)) {
        ...
        if (language.toLower() != "english") {
            ...
            m_currentJohnnyTranslator->load("johnny_" + m_languagesAndCodes[language.toLower()], ":/translations");
